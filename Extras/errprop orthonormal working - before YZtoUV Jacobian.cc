...
G4ThreeVector vyV(0, std::sin(rotationAngle), std::cos(rotationAngle));
...
void Initialize() 
{
  ...
  // G4UImanager::GetUIpointer()->ApplyCommand("/process/iverbose 4"); // GEANT4E specific verbose command, doesn't seem to work.
...
// Take the starting plane values, and add changes to similulate a non-perfect starting trajectory from some initial weak trackfitting algorithm.
  ...
  G4double startingVMom;// = YZtoVUcoordinateTransformationMatrix(0,0)*startingYMom + YZtoVUcoordinateTransformationMatrix(0,1)*startingZMom;
  G4double startingUMom;// = YZtoVUcoordinateTransformationMatrix(1,0)*startingYMom + YZtoVUcoordinateTransformationMatrix(1,1)*startingZMom;
...
// wzU = G4ThreeVector(0, -std::sin(rotationAngle), std::cos(rotationAngle));
  ...
  G4ErrorFreeTrajState* myTestFreeTrajState 
    = new G4ErrorFreeTrajState("e+", testx, testp, testerror );
...
for (int passNumber = 0; passNumber < numPasses; ++passNumber)
{
  ...
  startingVMom = YZtoVUcoordinateTransformationMatrix(0,0)*startingYMom + YZtoVUcoordinateTransformationMatrix(0,1)*startingZMom;
...
/////////////////////////////////////////////////////////////////////////////////////
            ...
            trackParamPredicted[1].at(planeNum) = (mySurfaceTrajState->GetParameters().GetPV()) / sqrt( 1./trackParamPredicted[0].at(planeNum)*1./trackParamPredicted[0].at(planeNum) - mySurfaceTrajState->GetParameters().GetPV()*mySurfaceTrajState->GetParameters().GetPV() - mySurfaceTrajState->GetParameters().GetPW()*mySurfaceTrajState->GetParameters().GetPW()  );
            trackParamPredicted[2].at(planeNum) = (mySurfaceTrajState->GetParameters().GetPW()) / sqrt( 1./trackParamPredicted[0].at(planeNum)*1./trackParamPredicted[0].at(planeNum) - mySurfaceTrajState->GetParameters().GetPV()*mySurfaceTrajState->GetParameters().GetPV() - mySurfaceTrajState->GetParameters().GetPW()*mySurfaceTrajState->GetParameters().GetPW()  );

            G4cout << " PV momentum: " << mySurfaceTrajState->GetParameters().GetPV() << " divided by PU momentum: " << sqrt( 1./trackParamPredicted[0].at(planeNum)*1./trackParamPredicted[0].at(planeNum) - mySurfaceTrajState->GetParameters().GetPV()*mySurfaceTrajState->GetParameters().GetPV() - mySurfaceTrajState->GetParameters().GetPW()*mySurfaceTrajState->GetParameters().GetPW() )
                   << " compared to P total momentum " << 1./trackParamPredicted[0].at(planeNum) << G4endl;
            ...
            // << " PU momentum: " << trackParamPredicted[2].at(planeNum) << G4endl
            // << " PV momentum: " << trackParamPredicted[1].at(planeNum) << G4endl   THIS stuff not quite right
            // << " U position: " << trackParamPredicted[4].at(planeNum) << G4endl
            // << " V position: " << trackParamPredicted[3].at(planeNum) << G4endl
...
/////////////////////////////////////////////////////////////////////////////////////
      ...
      /////////////////////////////////////////////////////////////////////////////////////
      double pvpOVERpup = deltaStartingTrack[1] + startingVMom/startingXMom; // For going to UV will have to change the last to pv over pu.
      double pwpOVERpup = deltaStartingTrack[2] + startingUMom/startingXMom;
      startingXMom = sqrt( reconstructedTotalMomentum*reconstructedTotalMomentum / (1 + pvpOVERpup*pvpOVERpup + pwpOVERpup*pwpOVERpup) );
      ...
      startingYPos = startingYPos + (YZtoVUcoordinateTransformationMatrixInverse(0,0)*deltaStartingTrack[3] + YZtoVUcoordinateTransformationMatrixInverse(0,1)*deltaStartingTrack[4]);
      startingZPos = startingZPos + (YZtoVUcoordinateTransformationMatrixInverse(1,0)*deltaStartingTrack[3] + YZtoVUcoordinateTransformationMatrixInverse(1,1)*deltaStartingTrack[4]);
...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  trackParamPredicted[1][0] = (backwardsSurfaceTrajState->GetParameters().GetPV())*trackParamPredicted[0][0]; // Not multiplying by the right thing on the end here.
...
//  If I'm mixing the UV basis (with a noHit entry) to the YZ basis, then I need to retain the full 2x2 error matrix with the proper A inverse on the left in order to properly reduce out the noHit entry that gets mixed in.            
 ...
 G4cout << " Hit U. " << G4endl;
#endif
              INVsigmaErrorMatrices[ipl](4,4) = 10000.; // Only fill the corresponding measured error matrix element.
            }
...
/////////////////////////////////////////////////////////////////////////////////////
    ...
    //             
            ...
            residuals[ipl] = paramMeasuredEigen[ipl]-paramPredictedEigen[ipl];
