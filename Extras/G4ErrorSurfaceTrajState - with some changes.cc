                  ...
                  const G4Vector3D& )
{
...
#ifdef G4EVERBOSE
  // if( iverbose >= 4) 
    G4cout << " SC2SD: vTN " << vTN << " vUN " << vUN << " vVN " << vVN << G4endl;
#endif
  ...
  // vectorU = vectorU / vectorU.mag(); // NICK

  G4double T1R = 1. / ( vTN * vectorU );

#ifdef G4EVERBOSE
 // if( iverbose >= 4) 
  G4cout << "surf vectors:U,V,W " << vectorU << " " <<  GetVectorV() << " " << GetVectorW() << "  T1R:"<<T1R<<G4endl;
#endif
...
#ifdef G4EVERBOSE
      // if( iverbose >= 4) 
        G4cout << GeV <<  " Q " << Q << " magHM " << magHM << " c_light/(km/ns) " << c_light/(km/ns) << G4endl;      
#endif
...
//Note: This are the inverse matrix elements of the matrix calculated above in a non-orthonormal detector basis.
G4double dxPerpdv = (1./ (vectorU.mag() * vectorU.mag())) * ( G4ThreeVector(GetVectorV()).dot(vUN) -  (GetVectorV()).dot(GetVectorW())*G4ThreeVector(GetVectorW()).dot(vUN) );
G4cout << "dxPerp over dv = " << dxPerpdv << G4endl;

G4double dxPerpdw = (1./ (vectorU.mag() * vectorU.mag())) * ( G4ThreeVector(GetVectorW()).dot(vUN) -  (GetVectorV()).dot(GetVectorW())*G4ThreeVector(GetVectorV()).dot(vUN) );
G4cout << "dxPerp over dw = " << dxPerpdw << G4endl;

G4double dyPerpdv = (1./ (vectorU.mag() * vectorU.mag())) * ( G4ThreeVector(GetVectorV()).dot(vVN) -  (GetVectorV()).dot(GetVectorW())*G4ThreeVector(GetVectorW()).dot(vVN) );
G4cout << "dyPerp over dv = " << dyPerpdv << G4endl;

G4double dyPerpdw = (1./ (vectorU.mag() * vectorU.mag())) * ( G4ThreeVector(GetVectorW()).dot(vVN) -  (GetVectorV()).dot(GetVectorW())*G4ThreeVector(GetVectorV()).dot(vVN) );
G4cout << "dyPerp over dw = " << dyPerpdw << G4endl;


G4double PU = sqrt( (1/(vectorU.mag()*vectorU.mag()*vectorU.mag()*vectorU.mag())) 
                          * (vectorU.mag()*vectorU.mag()*1./fTrajParam.GetInvP()*1./fTrajParam.GetInvP() 
                             - fTrajParam.GetPV()*fTrajParam.GetPV() - fTrajParam.GetPW()*fTrajParam.GetPW()
                             + 2 * GetVectorV().dot(GetVectorW() * fTrajParam.GetPV()*fTrajParam.GetPW())));
G4double vPrime = fTrajParam.GetPV()/PU;
G4double wPrime = fTrajParam.GetPW()/PU;

G4double normFactor = sqrt( 1 + vPrime*vPrime + wPrime*wPrime);
