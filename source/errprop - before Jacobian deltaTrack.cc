...
// Take the starting plane values, and add changes to similulate a non-perfect starting trajectory from some initial weak trackfitting algorithm.
  ...
  G4double initialXPosChange = -.01; // The starting plane is now right in front of the first plane hit and uses its recorded values as it's starting parameters. Making this too low causes errors, making it too large makes the results worse. 0.01 or 0.1 seems to be okay.
  ...
  G4double initialXMomChange =0200.*(r3.Rndm()-0.5);
  G4double initialYMomChange =0200.*(r3.Rndm()-0.5);
  G4double initialZMomChange =0200.*(r3.Rndm()-0.5);
...
/////////////////////////////////////////////////////////////////////////////////////
            // After getting to the target, once again create the surface trajectory state on the target.
                  ...
                  // I believe the last element in the SC2SDMatrixTransformation vector is the matrix by the same name on the plane that we just arrived at.
/////////////////////////////////////////////////////////////////////////////////////
/*                  Eigen::MatrixXd testSC2SDmatrix(5,5); // Fill that last matrix in to Eigen so we can easily multiply it against the extra Jacobian.

                  testSC2SDmatrix = Eigen::MatrixXd::Zero(5,5);

                   for(int i=0;i<5;i++) {
                     for(int j=0; j<5;j++) {
                          testSC2SDmatrix(i,j) = SC2SDTransformationMatrix.back()[i][j];

                     }
                   } 

                  Eigen::MatrixXd anotherMatrix = testSC2SDmatrix.inverse() * JacobianToVU;

                  G4cout << "Planenum: " << planeNum << " dxPerp over dv with extra Jacobian conversion: " << anotherMatrix(3,3) << G4endl;
*/
/////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////
      ...
      double sSin = std::sin(rotationAngle);
      double cCos = std::cos(rotationAngle);

      double pvpOVERpup = deltaStartingTrack[1] + startingVMom/startingXMom;
      double pwpOVERpup = deltaStartingTrack[2] + startingUMom/startingXMom;
      // startingXMom = sqrt( reconstructedTotalMomentum*reconstructedTotalMomentum / (1 + pvpOVERpup*pvpOVERpup + pwpOVERpup*pwpOVERpup) ); // This is only correct for an orthogonal system. Use below for a non-orthogonal system.
      startingXMom = sqrt( reconstructedTotalMomentum*reconstructedTotalMomentum / (1 + (1/(4*sSin*sSin*cCos*cCos)) * ( pvpOVERpup*pvpOVERpup + pwpOVERpup*pwpOVERpup) + (2*(sSin*sSin-cCos*cCos))/(4*sSin*sSin*cCos*cCos) * (pvpOVERpup * pwpOVERpup) ));
     ...
     //       =======================
            ...
            // Form the chi^2 for a single plane then add them all up to get a total chi^2 for the potential track - equation 23 in the GEANE manual.
             ...
             G4cout << "Chi^2 for the track is: " << chiSquaredTotal << G4endl;
              ...
              // I should be able to multiply the Jacobian against deltaPsiNought here to immediately convert back to YZ for easire use in the main code.
