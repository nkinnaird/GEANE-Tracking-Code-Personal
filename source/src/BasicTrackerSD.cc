...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(aStep->GetTrack()->GetTrackID() == 1 && pPreStepPoint->GetKineticEnergy() > energyThreshold)
...
strawVectorUV.setZ(transformInverse[2][0]*strawTranslationVector.x()+transformInverse[2][1]*strawTranslationVector.y()+transformInverse[2][2]*strawTranslationVector.z());
  ...
  G4cout << "Copy number preStepPoint: " <<  pPreStepPoint->GetPhysicalVolume()->GetCopyNo() << G4endl
    ...
    G4ThreeVector betterHitPosition;
    betterHitPosition.setX(strawTranslationVector.x());
    betterHitPosition.setY

    newHit->SetTrackID  (aStep->GetTrack()->GetTrackID());
    newHit->SetPos (betterHitPosition); // Will actually need to set the xyz positions of the straw better than this, from the straw vector or something, since the chord start will not be equal to the U or V plane, etc.
    newHit->SetMom (pPreStepPoint->GetMomentum()); // This one is probably approximately okay.
    newHit->SetGlobalTime (pPreStepPoint->GetGlobalTime()); 
    newHit->SetProperTime (pPreStepPoint->GetProperTime()); 
    newHit->SetCopyNo (pPreStepPoint->GetPhysicalVolume()->GetCopyNo()); // I probably need to modulus this to get a comparable layer number, and also for passing to errprop which uses copyNo to track from plane to plane.
